//reference:E.g., lock-chart solving and dependency management.
//reference:Usually based on conflict-driven clause learning (https://en.wikipedia.org/wiki/Conflict-driven clause learning), a fascinating algorithm worth reading about.
//reference:https://en.wikipedia.org/wiki/Slitherlink
//reference:https://www.chiark.greenend.org.uk/~sgtatham/puzzles/js/loopy.html
//reference:https://kwontomloop.com/
//reference:https://www.csie.ntu.edu.tw/~b07902134/ada-19hw4p1-public.zip
//reference: https://www.msoos.org/
		...
		for (int i = 0; i < edges.size(); i++){
				...
				while (!visited[e]) {
						...
						if (e <= c * (r + 1)){ //horizon
								...
								j2 = j1 + 1;
								//���I
								if (i1 <= r) neighbor.push_back(v[i1][j1]); //�U
								if (j1 % (c + 1) >= 2) neighbor.push_back(h[i1][j1 - 1]); //��
								if (i1 > 1) neighbor.push_back(v[i1 - 1][j1]); //�W
								//�k�I
								if (i2 <= r) neighbor.push_back(v[i2][j2]); //�U
								if (j2 <= c) neighbor.push_back(h[i2][j2]); //�k
								if (i2 > 1) neighbor.push_back(v[i2 - 1][j2]); //�W
						}
						else {
								i1 = (e - (c * (r + 1)) - 1) / (c + 1) + 1;
								j1 = (e - (c * (r + 1)) - 1) % (c + 1) + 1;
								...
								j2 = j1;
								//�W�I
								if (j1 <= c) neighbor.push_back(h[i1][j1]);//�k
								if ((j1 - 1) % (c + 1) >= 1) neighbor.push_back(h[i1][j1 - 1]); //��
								if (i1 > 1) neighbor.push_back(v[i1 - 1][j1]); //�W

								//�U�I
								if (i2 <= r) neighbor.push_back(v[i2][j2]); //�U
								if (j2 <= c) neighbor.push_back(h[i2][j2]);//�k
								if ((j2 - 1) % (c + 1) >= 1) neighbor.push_back(h[i2][j2 - 1]); //��
						}
